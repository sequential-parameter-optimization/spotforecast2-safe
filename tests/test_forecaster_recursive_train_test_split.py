...
def test_train_test_split_one_step_ahead_with_exog():
    """
    Test _train_test_split_one_step_ahead with exog.
    Adapted from skforecast.
    """
    y = pd.Series(
    ...
    expected_X_train = pd.DataFrame(
        {
            ...
            "exog_2": [1005.0, 1006.0, 1007.0, 1008.0, 1009.0],
        },
    ...
    expected_X_test = pd.DataFrame(
        {
            ...
            "exog_2": [1010.0, 1011.0, 1012.0, 1013.0, 1014.0],
        },
        index=pd.DatetimeIndex(
...
def test_train_test_split_one_step_ahead_no_exog():
    """
    Test _train_test_split_one_step_ahead without exog.
    """
    y = pd.Series(
        np.arange(15), index=pd.date_range("2020-01-01", periods=15), dtype=float
    )
