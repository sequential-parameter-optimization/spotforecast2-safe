...
class TestEntsoeDownloader(unittest.TestCase):
    ...
    def test_download_new_data_success(self, mock_client_class, mock_fetch, mock_get_home):
        ...
        mock_df = pd.DataFrame({"Actual": [123]}, index=[dates[-1] + pd.Timedelta(hours=1)])
        mock_client.query_load_and_forecast.return_value = mock_df
        
        # We need a large enough end date to pass the 24h check
    ...
    @patch("spotforecast2_safe.downloader.entsoe.fetch_data")
    @patch("spotforecast2_safe.downloader.entsoe.EntsoePandasClient")
    def test_download_new_data_cooldown(self, mock_client_class, mock_fetch, mock_get_home):
