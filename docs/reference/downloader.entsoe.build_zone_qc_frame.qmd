...
| data_home | [Optional](`typing.Optional`)\[[Union](`typing.Union`)\[[Path](`pathlib.Path`), [str](`str`)\]\] | Override for the data home directory (a ``Path``-like or ``str``).  When ``None``, ``get_data_home()`` is used (reads the ``SPOTFORECAST2_DATA`` environment variable). | `None`    |
