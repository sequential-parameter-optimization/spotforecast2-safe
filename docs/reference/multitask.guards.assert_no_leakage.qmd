...
|        | [RuntimeError](`RuntimeError`)                               | When ``mt.results[task][target]["forecaster"].estimator`` cannot be read or exposes none of the supported fitted-feature-name attributes (see `_fitted_feature_names`).  This signals a verifiability invariant violation rather than a leakage violation. |
