...
| metrics   | [tuple](`tuple`)\[[str](`str`), ...\]                                                          | Subset of `SUPPORTED_METRICS` to compute, in output order. ``"mae"``, ``"rmse"``, and ``"bias"`` are in the units of the series; ``"mape"`` is a percentage; ``"upr"`` is the under-prediction rate, the percentage of points with ``forecast < actual``. The ranking uses ``metrics[0]``. | `SUPPORTED_METRICS` |
