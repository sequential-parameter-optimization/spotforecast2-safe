...
|        | [Tuple](`typing.Tuple`)\[[pd](`pandas`).[DataFrame](`pandas.DataFrame`), [pd](`pandas`).[DataFrame](`pandas.DataFrame`)\] | - **weather_aligned** – Raw weather DataFrame reindexed to the same ``[start, cov_end]`` hourly grid (forward-filled). Also carries any columns requested via *derived_features* (e.g. ``hdh``, ``cdh``, ``apparent_temperature``, ``dew_point``), so they are selectable downstream via raw-weather membership in ``weather_aligned.columns``. |
