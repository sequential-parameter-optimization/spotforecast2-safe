...
def _backtesting_forecaster(
    ...
    Args:
        ...
        suppress_warnings (bool, optional): If `True`, spotforecast warnings will be
            ...
            Defaults to False.
        progress_desc (str | None, optional): Description used as the prefix of the
            per-fold ``tqdm`` progress bar (only applied when `show_progress` is
            `True`). The SpotOptim search passes a coarse "config k/N" label here so
            that the fine-grained fold bar also reports how many candidate
            configurations have been evaluated so far. If `None`, the bar is shown
            without a description, reproducing the previous behaviour.
            Defaults to None.
    ...
    if show_progress:
        folds = tqdm(folds, desc=progress_desc)
